        ...
        case DOUBLECOLON:
          ...
          case ARROW:
            ...
            lambdaBody = LambdaBody();
             ...
             {
               inner = ((EnclosedExpr)expr).getInner();
               if(inner != null && inner instanceof NameExpr)
               {
                 id = new VariableDeclaratorId(inner.getBeginLine(), inner.getBeginColumn(), inner.getEndLine(), inner.getEndColumn(), ((NameExpr)inner).getName(), 0);
                 params = add(params, new Parameter(expr.getBeginLine(), expr.getBeginColumn(), expr.getEndLine(), expr.getEndColumn(), 0, null, null, false, id));
                 expr = new LambdaExpr(expr.getBeginLine(), expr.getBeginColumn(), lambdaBody.getEndLine(), lambdaBody.getEndColumn(), params, lambdaBody, true);
               }
               else {
                 expr = new LambdaExpr(expr.getBeginLine(), expr.getBeginColumn(), lambdaBody.getEndLine(), lambdaBody.getEndColumn(), null, lambdaBody, true);

           }
             }
             else if( expr instanceof NameExpr)
             {
                   id = new VariableDeclaratorId(expr.getBeginLine(), expr.getBeginColumn(), expr.getEndLine(), expr.getEndColumn(), ((NameExpr)expr).getName(), 0);
               params = add(params, new Parameter(expr.getBeginLine(), expr.getBeginColumn(), expr.getEndLine(), expr.getEndColumn(), 0, null, null, false, id));
               expr = new LambdaExpr(expr.getBeginLine(), expr.getBeginColumn(), expr.getEndLine(), expr.getEndColumn(), params, lambdaBody, false);
             }
             else if( expr instanceof LambdaExpr)
             {
               ((LambdaExpr)expr).setBody(lambdaBody);
               expr.setEndLine(lambdaBody.getEndLine());
               expr.setEndColumn(lambdaBody.getEndColumn());
             }
             else
             {
               expr = null;
...
/* Annotation Types. */
  ...
  public ASTParser(InputStream stream) {
     this(stream, null);
  }
  /** Constructor with InputStream and supplied encoding */
  public ASTParser(InputStream stream, String encoding) {
  ...
  /** Constructor. */
  public ASTParser(Reader stream) {
