...
public class EqualsVisitorsPackageTest extends AbstractEqualsVisitorsTest {
    ...
    void equals_differentPackage_false(Strategy strategy) {
        assertNotEqualAfterMutation(
                PACKAGE,
                () -> nodeRight.getPackageDeclaration().get().getName(),
                n -> n.setIdentifier(n.getIdentifier() + ".differentName"),
                strategy);
    }
    ...
    void equals_differentPackageAnnotation_false(Strategy strategy) {
        assertNotEqualAfterMutation(
                PACKAGE,
                () -> nodeRight.getPackageDeclaration().get(),
                n -> n.getAnnotations().remove(0),
                strategy);
    }
