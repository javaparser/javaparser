...
public class EqualsVisitorsMiscTest extends AbstractEqualsVisitorsTest {
    ...
    void equals_differentReceiverParameterAnnotation_false(Strategy strategy) {
        assertNotEqualAfterMutation(
                RECEIVER_CODE,
                () -> nodeRight.findFirst(ReceiverParameter.class).get(),
    ...
    void equals_differentReceiverParameterName_false(Strategy strategy) {
        assertNotEqualAfterMutation(
                RECEIVER_CODE,
                () -> nodeRight.findFirst(ReceiverParameter.class).get(),
                n -> n.setName(new Name("Different.this")),
                strategy);
    }
    ...
    void equals_differentModifierKeyword_false(Strategy strategy) {
        assertNotEqualAfterMutation(
                MODIFIER_CODE,
                () -> nodeRight.findFirst(Modifier.class).get(),
                n -> n.setKeyword(Modifier.Keyword.PRIVATE),
                strategy);
    }
    ...
    void equals_differentModifierComment(Strategy strategy) {
        assertCommentChangeHandled(
                MODIFIER_CODE, () -> nodeRight.findFirst(Modifier.class).get(), strategy);
    }
    ...
    void equals_differentNodeListSize_false(Strategy strategy) {
        assertNotEqualAfterMutation(
                NODELIST_CODE,
                () -> nodeRight.findFirst(MethodDeclaration.class).get(),
                n -> n.getParameters().add(new Parameter(new ClassOrInterfaceType("String"), "c")),
                strategy);
    }
    ...
    /**
     * Direct-node regression test for #5056: NoCommentEqualsVisitor should ignore
     * MarkdownComment content when comparing comment nodes directly.
     *
     * WARNING: Before enabling this test, check whether PR #5057 has already been
     * merged and this assertion is covered by NoCommentEqualsVisitorTest to avoid
     * duplication.
     *
     * @see <a href="https://github.com/javaparser/javaparser/issues/5056">#5056</a>
     */
    @Disabled("Blocked by #5056 — MarkdownComment content is incorrectly compared in NoCommentEqualsVisitor")
    @Test
    void noCommentEquals_directMarkdownCommentNodes_shouldIgnoreContent() {
        assertTrue(
                NoCommentEqualsVisitor.equals(new MarkdownComment("a"), new MarkdownComment("b")),
                "NoCommentEqualsVisitor should return true for MarkdownComment nodes with different content");
    }
}
