...
public class EqualsVisitor implements GenericVisitor<Boolean, Visitable> {
    ...
    public Boolean visit(final JmlInfFlowClause n, final Visitable arg) {
        ...
        if (!nodesEquals(n.getBy(), n2.getBy()))
            return false;
        if (!nodesEquals(n.getDeclassifies(), n2.getDeclassifies()))
            return false;
        if (!nodesEquals(n.getErases(), n2.getErases()))
            return false;
        if (!nodesEquals(n.getExpressions(), n2.getExpressions()))
            return false;
        if (!objEquals(n.getKind(), n2.getKind()))
            return false;
        if (!nodesEquals(n.getNewObjects(), n2.getNewObjects()))
            return false;
        if (!nodeEquals(n.getName(), n2.getName()))
            return false;
        if (!nodesEquals(n.getAssociatedSpecificationComments(), n2.getAssociatedSpecificationComments()))
