...
public final class ForEachStmt extends Statement implements NodeWithBody<ForEachStmt> {
    ...
    public ForEachStmt setVariable(final VariableDeclarationExpr variable) {
        ...
        if (variable.getVariables().size() != 1) {
            throw new IllegalArgumentException("A foreach statement's variable declaration must have exactly one " +
                                               "variable declarator. Given: " + variable.getVariables().size() + ".");
        }
