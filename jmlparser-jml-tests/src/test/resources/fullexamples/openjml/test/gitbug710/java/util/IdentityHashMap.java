    ...
    implements Map<K,V>, java.io.Serializable, Cloneable
{
    ...
    //@ public invariant table.length > 1 && table.length % 2 == 0;
    ...
    //@  ensures \result == (key == null ? NULL_KEY : key);
     ...
     */
    //@ private normal_behavior
    //@  ensures \result == (key == NULL_KEY ? null : key);
    //@ nullable
     ...
     */
    //@ private normal_behavior
    //@   requires MINIMUM_CAPACITY <= initCapacity <= MAXIMUM_CAPACITY; // TODO - power of two
    //@   ensures table.length >1 && table.length % 2 == 0 && table.length == initCapacity *2;
    //@ helper
    ...
    //@   requires m.size() <= MAXIMUM_CAPACITY/2;
     ...
     */
    //@ private normal_behavior
    //@  requires length > 1; // length a power of 2
     ...
     */
    //@ private normal_behavior
    //@  requires 0 <= i < len-1 && i%2 == 0 && len%2 == 0; // len is power of two
    //@  ensures 0 <= \result < len-1 && \result %2 == 0;
    ...
    public V get(Object key) {
        ...
        int len = tab.length;
        int i = hash(k, len);
        //@ loop_invariant 0 <= i < len-1 && i%2 == 0;
        //@ loop_modifies i;
        while (true) {
            Object item = tab[i];
            if (item == k)
    ...
    public boolean containsValue(Object value) {
        Object[] tab = table;
        //@ loop_invariant 1 <= i <= tab.length+1 && i%2 == 1;
        //@ loop_modifies i;
    ...
    public V put(K key, V value) {
        ...
        //@ loop_invariant modCount >= 0;
        //@ loop_modifies modCount, table, table[*];
        for (;;) {
            ...
            //@ assume modCount < Integer.MAX_VALUE;
            modCount++;
            tab[i] = k;
            tab[i + 1] = value;
            size = s;
     ...
     */
    //@ private normal_behavior
    //@   requires 0 < newCapacity <= MAXIMUM_CAPACITY ;
    //@   assigns table;
    //@   ensures \result ==> (table.length % 2 == 0 && table.length == 2 * newCapacity);
    //@   ensures !\result ==> table == \old(table);
    ...
    public V remove(Object key) {
        ...
        while (true) {
            ...
            if (item == k) {
                ...
                size--;
                @SuppressWarnings("unchecked")
                    /*@ nullable */V oldValue = (V) tab[i + 1];
                tab[i + 1] = null;
                tab[i] = null;
                closeDeletion(i);
                //@ ghost \bigint szold = \old(((Map)this).size);
                //@ assume oldValue != null ==> ((Map)this).size == (\old(keys[key.uniqueHash]) ? szold-1 : szold);
                //@ assume oldValue != null ==> keys[key.uniqueHash] == false;
                return oldValue;
            }
            if (item == null)
                return null;
