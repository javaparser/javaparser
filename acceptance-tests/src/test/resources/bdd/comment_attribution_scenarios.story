...
Given the class:
package japa.japa.parser.comments;
...
When the class is parsed by the Java japa.parser
...
Given the class:
package japa.japa.parser.javacc;
public class Teste {
    //line comment1
...
When the do not consider annotations as node start for code attribution is true on the Java japa.parser
When the class is parsed by the Java japa.parser
...
When the do not consider annotations as node start for code attribution is false on the Java japa.parser
When the class is parsed by the Java japa.parser
Then the compilation unit has 1 contained comments
Then comment 1 in compilation unit is not an orphan
