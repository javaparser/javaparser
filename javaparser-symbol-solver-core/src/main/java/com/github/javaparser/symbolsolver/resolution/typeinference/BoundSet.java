...
public class BoundSet {
    ...
    public BoundSet deriveImpliedBounds(TypeSolver typeSolver) {
        ...
        newConstraintsSet = forEachPairSameAs(
        ...
        newConstraintsSet = forEachPairSameAndSubtype(
                (a, b, currentConstraintSet) -> {
                    if (areSameTypeInference(a.getS(), b.getS())) {
                        currentConstraintSet = currentConstraintSet.withConstraint(
        ...
        newConstraintsSet = forEachPairSubtypeAndSubtype(
                (a, b, currentConstraintSet) -> {
                    if (areSameTypeInference(a.getT(), b.getS())) {
                        currentConstraintSet = currentConstraintSet.withConstraint(
                                new TypeSubtypeOfType(typeSolver, b.getS(), a.getT()));
        ...
        newConstraintsSet = forEachPairSameAs(
                (a, b, currentConstraintSet) -> {
                    if (a.getS().isInferenceVariable() && isProperType(a.getT())) {
                        ...
                        currentConstraintSet =
                                currentConstraintSet.withConstraint(new TypeSameAsType(sub.apply(S), sub.apply(T)));
                    }
                        ...
                        if (a.getT().isReferenceType() && b.getT().isReferenceType()) {
                            ...
                            for (Pair<ResolvedReferenceType, ResolvedReferenceType> pair : pairs) {
                                for (int i = 0;
                                        ...
                                        i++) {
                                    ResolvedType si =
                                            pair.a.typeParametersValues().get(i);
                                    ResolvedType ti =
                                            pair.b.typeParametersValues().get(i);
        ...
        for (Bound b :
            ...
            // parameters. Let θ represent the substitution [P1:=α1, ..., Pn:=αn]. Let R be a type that is not an
            // inference
            // variable (but is not necessarily a proper type).
            //
            // A set of bounds on α1, ..., αn is implied, constructed from the declared bounds of P1, ..., Pn as
            // specified
            // in §18.1.3.
            //
        ...
        for (Set<InferenceVariable> alphas : allSetsWithProperty(uninstantiatedPortionOfV, dependencies)) {
            ...
            if (!hasSomeCaptureForAlphas) {
                ...
                for (InferenceVariable alphaI : alphas) {
                    ...
                    //   - Otherwise, if the bound set contains throws αi, and the proper upper bounds of αi are, at
                    // most,
                ...
                //   Otherwise, the result contains the bound false, so a second attempt is made to instantiate { α1,
                // ..., αn }
            ...
            // - If the bound set contains a bound of the form G<..., αi, ...> = capture(G<...>) for some i (1 ≤ i ≤ n),
            // or;
                ...
                //
                //   - For all i (1 ≤ i ≤ n), if αi has one or more proper lower bounds L1, ..., Lk, then let the lower
                // bound
                ...
                //
                //   If the type variables Y1, ..., Yn do not have well-formed bounds (that is, a lower bound is not a
                // subtype
                //   of an upper bound, or an intersection type is inconsistent), then resolution fails.
                //
                //   Otherwise, for all i (1 ≤ i ≤ n), all bounds of the form G<..., αi, ...> = capture(G<...>) are
                // removed
