...
public class LambdaThrowsCompatibleWithType extends ConstraintFormula {
    ...
    public ReductionResult reduce(BoundSet currentBoundSet) {
        ...
        //
        // - Otherwise, let the target function type for the lambda expression be determined as specified in §15.27.3.
        // If no valid function type can be found, the constraint reduces to false.
        //
        // - Otherwise, if the lambda expression is implicitly typed, and one or more of the function type's parameter
        ...
        //
        // - Otherwise, if the function type's return type is neither void nor a proper type, the constraint reduces to
        // false.
        //
        //   This condition never arises in practice, due to the substitution applied to the target type in §18.5.2.
        //
        // - Otherwise, let E1, ..., En be the types in the function type's throws clause that are not proper types. If
        // the lambda expression is implicitly typed, let its parameter types be the function type's parameter types. If
        // the lambda body is a poly expression or a block containing a poly result expression, let the targeted return
        // type be the function type's return type. Let X1, ..., Xm be the checked exception types that the lambda body
        // can throw (§11.2). Then there are two cases:
        //
