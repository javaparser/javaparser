...
//                removeImports(compilationUnit);
    ...
    public void removeImports(CompilationUnit compilationUnit) {
        // FIXME: Fix this (causes java.util.ConcurrentModificationException)
        // Remove the import. 
    ...
    public void removeAnnotations(CompilationUnit compilationUnit) {
        // TODO: Consider -- should this likely be a replace operation,
        //  e.g. replace it with `@OutdatedGeneration` or similar that can be searched for?
    ...
    protected List<CompilationUnit> getParsedCompilationUnitsFromSourceRoot(SourceRoot sourceRoot) throws IOException {
        ...
        boolean allParseResultsSuccessful = parseResults.stream().allMatch(ParseResult::isSuccessful);
        if (!allParseResultsSuccessful) {
